...
smplx_pkl_dir='./2020-06-11-10-06-48'
smplx_pkl='001_all.pkl'
smplx_parms = pickle.load(open(os.path.join(smplx_pkl_dir,smplx_pkl), 'rb'))

smpl_pkl_dir='./output'
smpl_pkl='001.pkl'
smpl_parms = pickle.load(open(os.path.join(smpl_pkl_dir,smpl_pkl), 'rb'))
...
for filename in os.listdir(folder):
    if filename.endswith('.pkl'):
        file_i = open(os.path.join(folder,filename), "rb")
...
pose_matrix_i=smpl_parms['full_pose'].cpu().detach().numpy().reshape(-1, 3, 3)
r = R.from_matrix(pose_matrix_i)
pose_rotvec=r.as_rotvec()


print(str(pose_rotvec.reshape(-1)).replace('  ',',').replace(' ',','))
print(str(smpl_shapes).replace('  ',','))
print(smpl_parms['betas'].cpu().detach().numpy())
#
# # step0:
